...
# 16 "Proyecto1.s" 2
 ...
 bcf ((INTCON) and 07Fh), 2 ;Colocamos en cero la bandera del TMR0
    endm
 ...
 movwf STATUS_TEMP ;Movemos lo que hay en el acumulador al registro
    isr:
        BANKSEL PORTB
 btfsc ((INTCON) and 07Fh), 0 ;Verificamos si alguno de los puertos de B cambiaron de estado
 call int_iocb ;Si, sí cambió de estado, llamamos a nuestra función
 btfsc ((INTCON) and 07Fh), 2 ;Verificamos si la bandera del TMR0 está encendida
 ...
 bsf IOCB, UP ;Habilitamos la interrupción al cambiar el estado de ((PORTB) and 07Fh), 6
 bsf IOCB, DOWN ;Habilitamos la interrupción al cambiar el estado de ((PORTB) and 07Fh), 7
 bsf IOCB, UP1 ;Habilitamos la interrupción al cambiar el estado de ((PORTB) and 07Fh), 4
 bsf IOCB, DOWN1 ;Habilitamos la interrupción al cambiar el estado de ((PORTB) and 07Fh), 5
 bsf IOCB, 0 ;Habilitamos la interrupción al cambiar el estado de ((PORTB) and 07Fh), 0

 banksel PORTB
 movf PORTB, W ;al leer termina la condición de mismatch
 bcf ((INTCON) and 07Fh), 0 ;Ponemos en cero el cambio de estado para que se reinicie la verificación
 ...
 bsf ((OSCCON) and 07Fh), 6 ;OSCCON 6 configuramos el bit 2 del IRCF como 1
 bsf ((OSCCON) and 07Fh), 5 ;OSCCON 5 configuramos el bit 1 del IRCF como 1
 bsf ((OSCCON) and 07Fh), 4 ;OSCCON 4 configuramos el bit 0 del IRCF como 1
 ...
 return ;Retorna a donde fue llamada esta función
    ...
    config_int_enable:
 bsf ((INTCON) and 07Fh), 5 ;INTCON ;Habilitamos la interrupción del TMR0
 bcf ((INTCON) and 07Fh), 2 ;INTCON ;Ponemos en cero la bandera del TMR0
 bsf ((INTCON) and 07Fh), 7 ;INTCON ;Habilitamos las interrupciones en general
 bsf ((INTCON) and 07Fh), 3 ;INTCON ;Habilitamos la interrupción del cambio en el puerto B
 bcf ((INTCON) and 07Fh), 0 ;INTCON ;Ponemos en cero la bandera del cambio de estado para que se reinicie la verificación
 return ;Retorna a donde fue llamada esta función
...
# 742 "Proyecto1.s"
 ...
 bcf ((INTCON) and 07Fh), 0 ;Limpiamos la bandera de interrupción del puerto B
 return ;Retornamos de nuestra función
